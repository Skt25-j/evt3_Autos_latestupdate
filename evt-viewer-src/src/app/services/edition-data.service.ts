...
export class EditionDataService {
  ...
  private loadAndParseEditionData() {
    // Support both the plain-string editionUrls format and the object format
    // ({ type, value, enable }) used by this edition's file_config.json.
    const first: any = this.editionUrls[0];
    const editionUrl: string = (first && typeof first === 'object') ? first.value : first;
