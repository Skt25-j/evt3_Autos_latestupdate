...
import { evtFindNearestPage, evtGetOwnerDoc } from './evt-custom-pages.util';
    ...
    public currentPage$ = merge(
        merge(
            ...
            mergeMap((id) => this.evtModelService.pages$.pipe(
                map((pages) => !id ? pages[0] : pages.find((p) => p.id === id) || evtFindNearestPage(id, pages, evtGetOwnerDoc(pages)))),
            ),
        ),
