...
export class EVTModelService {
  ...
  public readonly rawPages$: Observable<Page[]> = this.editionSource$.pipe(
    map((source) => this.editionStructureParser.parsePages(source).pages),
    filter((pages) => pages.length > 0),
    shareReplay(1),
  );

  // Custom: the effective page list can be overridden (transpositions +
  // blank-page filtering for the critical view). See evt-custom-pages.util.
